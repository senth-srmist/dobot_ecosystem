...
class Dobot:
    ...
    def get_ir(self, port: GPIO = GPIO.PORT_GP4) -> bool:
        ...
        # DobotEDU uses:
        # GP1 -> 1 -> serial parameter 0
        # GP2 -> 2 -> serial parameter 1
        # GP4 -> 4 -> serial parameter 3
        #
        # DobotAPI GPIO enum is zero-based, so add 1.
        dobot_port = int(port) + 1
    
        msg.params.extend(bytearray([dobot_port]))
        msg.params.extend(bytearray([0x01]))  # enable
        msg.params.extend(bytearray([0x01]))  # Version 2
        ...
        if not response.params:
            raise DobotException("Empty response from IR sensor")
    ...
    def pause(self) -> None:
        """Stop execution of queued commands after the current command."""
        self._set_queued_cmd_stop_exec()


    def resume(self) -> None:
        """Resume execution of queued commands."""
        self._set_queued_cmd_start_exec()
    
    
    def force_stop(self) -> None:
        """Immediately stop the currently executing command."""
    ...
    def _set_queued_cmd_force_stop_exec(self) -> Message:
        msg = Message()
        msg.id = 242
        msg.ctrl = 0x01
        return self._send_command(msg)
